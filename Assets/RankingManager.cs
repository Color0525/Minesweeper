...
public class RankingManager : MonoBehaviour
{
    ...
    public void GetRanking(float score)
    {
        ...
        NCMBQuery<NCMBObject> query = new NCMBQuery<NCMBObject>("MinesweeperHighScore");
        ...
        query.FindAsync((List<NCMBObject> objList, NCMBException e) =>
        {
            ...
            else
            {
                ...
                if ((score > 0 && m_ranking.Count < 10) || score > float.Parse(m_ranking[m_ranking.Count - 1]["Score"].ToString()) || m_ranking.Count == 0)
                {
    ...
    void MakeRankingText()
    {
        ...
        for (int i = 0; i < m_ranking.Count; i++)
        {
            ...
            float time = float.Parse(m_ranking[i]["Score"].ToString());
            int minute = (int)time / 60;
            float second = time % 60;
            string score = $"{ minute.ToString("00")}.{ second.ToString("00.00")}";
    ...
    public void SetScoreOfCurrentPlay(float score)
    {
        GetRanking(score);
    }
    ...
    {
        // 保存するためのデータを作る
        NCMBObject obj = new NCMBObject("MinesweeperHighScore");
